...
impl YieldFarmingContract {
    ...
    pub fn stake_tokens(env: Env, staker: Address, pool_id: u32, amount: i128) {
        ...
        env.storage()
            .persistent()
            .set(&DataKey::Stake(staker.clone(), stake_id), &position);
        env.storage()
            .persistent()
            .set(&DataKey::StakeCounter(staker.clone()), &stake_id);

        let mut user_stakes: Vec<u32> = env
            .storage()
            .persistent()
    ...
    pub fn calculate_rewards(env: Env, staker: Address, stake_id: u32) -> i128 {
        ...
        let base_reward =
            (position.amount * (pool.apy_basis_points as i128) * (time_elapsed as i128))
    ...
    pub fn claim_rewards(env: Env, staker: Address, stake_id: u32) -> i128 {
        ...
        env.storage()
            .persistent()
            .set(&DataKey::PoolStats(position.pool_id), &stats);
    ...
    pub fn unstake(env: Env, staker: Address, stake_id: u32) -> i128 {
        ...
        env.storage()
            .persistent()
            .remove(&DataKey::Stake(staker.clone(), stake_id));
    ...
    fn update_pool_stats(env: &Env, pool_id: u32, amount: i128, is_stake: bool) {
        ...
        env.storage()
            .persistent()
            .set(&DataKey::PoolStats(pool_id), &stats);
    }
}
