...
impl BattlePassContract {
    ...
    pub fn purchase_pass(env: Env, buyer: Address, season_id: u32) {
        ...
        if env
            .storage()
            .persistent()
            .has(&DataKey::PlayerPass(buyer.clone(), season_id))
        {
        ...
        env.storage()
            .persistent()
            .set(&DataKey::BattlePass(pass_id), &battle_pass);
        env.storage()
            .persistent()
            .set(&DataKey::PlayerPass(buyer, season_id), &pass_id);

        // Increment pass ID counter
        env.storage()
            .persistent()
            .set(&DataKey::NextPassId, &(pass_id + 1));
    }
    ...
    pub fn add_xp(env: Env, pass_id: u32, amount: u32) {
        ...
        env.storage()
            .persistent()
            .set(&DataKey::BattlePass(pass_id), &updated_pass);
    }
    ...
    pub fn claim_tier_reward(env: Env, pass_id: u32, tier_index: u32) -> PassTier {
        ...
        env.events().publish(
            ...
            (
                tier_index,
                tier_reward.reward_amount,
                tier_reward.reward_type as u32,
            ),
        );
    ...
    pub fn get_pass(env: Env, pass_id: u32) -> (u32, u32, Vec<u32>) {
        ...
        (
            battle_pass.xp,
            battle_pass.tier_reached,
            battle_pass.rewards_claimed,
        )
    }
