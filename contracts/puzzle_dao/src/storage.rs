use soroban_sdk::{Env, Address, Vec, Val, Map};
use crate::types::{DataKey, GovernanceConfig, Proposal, Member, TreasuryInfo, MembershipTier};
...
pub fn set_member(env: &Env, member: &Member) {
    env.storage().persistent().set(&DataKey::Member(member.address.clone()), member);
}
...
pub fn get_member_count(env: &Env) -> u64 {
    env.storage().instance().get(&DataKey::MemberCount).unwrap_or(0)
}

pub fn increment_member_count(env: &Env) -> u64 {
...
pub fn set_treasury_info(env: &Env, treasury: &TreasuryInfo) {
    env.storage().instance().set(&DataKey::TreasuryInfo, treasury);
}

pub fn get_treasury_info(env: &Env) -> TreasuryInfo {
    env.storage().instance().get(&DataKey::TreasuryInfo).unwrap_or(TreasuryInfo {
        total_balance: 0,
        allocated_funds: 0,
        last_distribution: 0,
    })
...
pub fn get_membership_thresholds(env: &Env) -> Map<MembershipTier, i128> {
    env.storage().instance().get(&DataKey::MembershipThresholds).unwrap_or_else(|| {
        let mut thresholds = Map::new(env);
        thresholds.set(MembershipTier::Basic, 1000);
        thresholds.set(MembershipTier::Active, 5000);
        thresholds.set(MembershipTier::Premium, 20000);
        thresholds.set(MembershipTier::Council, 100000);
        thresholds
    })
}

pub fn set_membership_thresholds(env: &Env, thresholds: &Map<MembershipTier, i128>) {
    env.storage().instance().set(&DataKey::MembershipThresholds, thresholds);
}
