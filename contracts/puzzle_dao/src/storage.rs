...
pub fn set_member(env: &Env, member: &Member) {
    env.storage()
        .persistent()
        .set(&DataKey::Member(member.address.clone()), member);
}
...
pub fn set_treasury_info(env: &Env, treasury: &TreasuryInfo) {
    env.storage()
        .instance()
        .set(&DataKey::TreasuryInfo, treasury);
}

pub fn get_treasury_info(env: &Env) -> TreasuryInfo {
    env.storage()
        .instance()
        .get(&DataKey::TreasuryInfo)
        .unwrap_or(TreasuryInfo {
            total_balance: 0,
            allocated_funds: 0,
            last_distribution: 0,
        })
...
pub fn get_membership_thresholds(env: &Env) -> Map<MembershipTier, i128> {
    env.storage()
        .instance()
        .get(&DataKey::MembershipThresholds)
        .unwrap_or_else(|| {
