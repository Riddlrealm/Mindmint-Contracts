...
impl DynamicNftContract {
    pub fn initialize(env: Env, admin: Address, oracle: Address) {
        ...
        env.storage().persistent().set(
            &DataKey::EvolutionRules,
            &Map::<u64, EvolutionRule>::new(&env),
        );
    }
    ...
    pub fn mint(env: Env, owner: Address) -> u32 {
        ...
        env.storage()
            .persistent()
            .set(&DataKey::DynamicNft(next_id), &nft);
        env.storage()
            .persistent()
            .set(&DataKey::NextTokenId, &(next_id + 1));

        env.events()
            .publish((symbol_short!("minted"),), (next_id, owner));
    ...
    pub fn add_xp(env: Env, oracle: Address, token_id: u32, amount: u64) {
        ...
        env.storage()
            .persistent()
            .set(&DataKey::DynamicNft(token_id), &nft);

        env.events()
            .publish((symbol_short!("xp_add"),), (token_id, amount, total_xp));
    ...
    pub fn add_evolution_rule(
        ...
        let mut rules: Map<u64, EvolutionRule> = env
            .storage()
            .persistent()
        ...
        env.storage()
            .persistent()
            .set(&DataKey::EvolutionRules, &rules);
    }
    ...
    pub fn update_oracle(env: Env, admin: Address, new_oracle: Address) {
        ...
        // Remove old oracle
        let old_oracle: Option<Address> = env
            .storage()
            .persistent()
    ...
    fn check_and_evolve(env: &Env, token_id: u32) {
        ...
        let rules: Map<u64, EvolutionRule> = env
            .storage()
            .persistent()
            .get(&DataKey::EvolutionRules)
            .unwrap_or_else(|| Map::new(env));
        ...
        if let Some(rule) = applicable_rule {
            ...
            env.events().publish(
                ...
                (token_id, old_level, rule.new_level, rule.new_metadata_uri),
            );
        }
    }
