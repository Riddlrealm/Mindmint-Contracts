...
pub fn set_global_lp_staked(env: &Env, amount: i128) {
    env.storage()
        .instance()
        .set(&DataKey::GlobalLpStaked, &amount);
}

pub fn get_global_lp_staked(env: &Env) -> i128 {
    env.storage()
        .instance()
        .get(&DataKey::GlobalLpStaked)
        .unwrap_or(0)
}
...
pub fn set_position(env: &Env, provider: &Address, pos: &MiningPosition) {
    env.storage()
        .persistent()
        .set(&DataKey::Position(provider.clone()), pos);
}
...
pub fn set_has_claimed(env: &Env, provider: &Address, epoch_id: u32) {
    env.storage()
        .persistent()
        .set(&DataKey::Claimed(provider.clone(), epoch_id), &true);
}

pub fn has_claimed(env: &Env, provider: &Address, epoch_id: u32) -> bool {
    env.storage()
        .persistent()
        .get(&DataKey::Claimed(provider.clone(), epoch_id))
        .unwrap_or(false)
}
